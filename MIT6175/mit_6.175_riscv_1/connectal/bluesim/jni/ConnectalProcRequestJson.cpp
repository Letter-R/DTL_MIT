...
{
    Json::Value request;
    request.append(Json::Value("softReset"));
    

    std::string requestjson = Json::FastWriter().write(request);;
    connectalJsonSend(p, requestjson.c_str(), (int)CHAN_NUM_ConnectalProcRequest_softReset);
    return 0;
};
...
ConnectalProcRequestCb ConnectalProcRequestJsonProxyReq = {
    ...
    ConnectalProcRequestJson_softReset,
};
